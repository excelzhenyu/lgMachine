 ...
 * @version 2016-06-13
 */
public class LgDeviceMonthStatics extends DataEntity<LgDeviceMonthStatics> {
	
	private static final long serialVersionUID = 1L;
	private Date insertTime;		// 写入时间
	private String deviceNo;		// 整机对外发布的编码
	private Integer workMonthId;		// 工作月度ID
	private String workMonthName;		// 工作月度名称
	private Integer workCount;		// 当月工作天数统计
	private Date workBegin;		// 当月初次开机时间
	private Date workStop;		// 当月最后一次工作结束时间
	private Integer sliceRunDuration;		// 开机时长
	private Integer sliceWorkDuration;		// 工作时长
	private Integer runOffCount;		// 当月开机次数
	...
	public LgDeviceMonthStatics() {
		super();
	}

	public LgDeviceMonthStatics(String id){
		super(id);
	}
	...
	public Integer getWorkMonthId() {
		return workMonthId;
	}

	public void setWorkMonthId(Integer workMonthId) {
		this.workMonthId = workMonthId;
	}
	
	@Length(min=0, max=20, message="工作月度名称长度必须介于 0 和 20 之间")
